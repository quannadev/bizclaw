...
impl PlatformDb {
    ...
    pub fn cloud_delete_tenant(&self, tenant_id: &str) -> Result<()> {
        self.conn
            .execute(
                "DELETE FROM cloud_tenants WHERE id = ?1",
                params![tenant_id],
            )
            .map_err(|e| BizClawError::Memory(format!("Delete tenant: {e}")))?;
        Ok(())
    ...
    pub fn cloud_get_config(&self) -> Result<serde_json::Value> {
        let mut stmt = self
            .conn
            .prepare("SELECT key, value FROM cloud_config")
            .map_err(|e| BizClawError::Memory(format!("Prepare: {e}")))?;
        let rows = stmt
            .query_map([], |row| {
...
mod tests {
    ...
    fn test_user_crud() {
        ...
        let id = db
            .create_user("test@example.com", hash, "admin", None)
            .unwrap();

        let user = db.get_user_by_email("test@example.com").unwrap();
        assert!(user.is_some());

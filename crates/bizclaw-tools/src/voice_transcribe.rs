...
impl VoiceTranscribeTool {
    ...
    async fn transcribe_whisper(&self, audio_path: &str, language: &str) -> Result<String> {
        let api_key = std::env::var("OPENAI_API_KEY").map_err(|_| {
            BizClawError::Tool("OPENAI_API_KEY not set for Whisper transcription".into())
        })?;
    ...
    async fn transcribe_groq(&self, audio_path: &str, language: &str) -> Result<String> {
        let api_key = std::env::var("GROQ_API_KEY").map_err(|_| {
            BizClawError::Tool("GROQ_API_KEY not set for Groq transcription".into())
        })?;
    ...
    async fn transcribe_local(&self, audio_path: &str, language: &str) -> Result<String> {
        ...
        if !output.status.success() {
            ...
            return Err(BizClawError::Tool(
                "Local whisper transcription failed".into(),
            ));
        }
    ...
    async fn execute(&self, arguments: &str) -> Result<ToolResult> {
        ...
        if !["wav", "m4a", "ogg", "mp3", "mp4", "webm", "flac"].contains(&ext.as_str()) {
            return Ok(ToolResult {
                tool_call_id: String::new(),
                output: format!(
                    "Error: Unsupported audio format '.{ext}'. Supported: wav, m4a, ogg, mp3, mp4, webm, flac"
                ),
                success: false,
            });
        }
        ...
        let output = match action {
            "transcribe" => {
                format!("📝 **Transcription** ({provider}, {size_mb:.1} MB)\n\n{transcript}")
            }
            "recap" => {
...
mod tests {
    ...
    async fn test_missing_file() {
        let tool = VoiceTranscribeTool::new();
        let result = tool
            .execute(r#"{"audio_path": "/nonexistent/audio.wav"}"#)
            .await
            .unwrap();
    ...
    async fn test_unsupported_format() {
        ...
        let tool = VoiceTranscribeTool::new();
        let result = tool
            .execute(&format!(r#"{{"audio_path": "{tmp}"}}"#))
            .await
            .unwrap();
        assert!(!result.success);

...
impl TaskDag {
    ...
    pub fn add_dependency(&mut self, dependent_id: &str, dependency_id: &str) -> Result<(), String> {
        let dep_idx = *self.node_map.get(dependency_id).ok_or("Dependency not found")?;
        let target_idx = *self.node_map.get(dependent_id).ok_or("Dependent task not found")?;
        ...
        if algo::is_cyclic_directed(&self.graph) {
            self.graph.remove_edge(self.graph.find_edge(dep_idx, target_idx).unwrap());
    ...
    pub fn get_ready_tasks(&mut self) -> Vec<String> {
        ...
        for idx in self.graph.node_indices() {
             if self.graph[idx].status == TaskStatus::Pending {
                 ...
                 let mut incoming = self.graph.neighbors_directed(idx, petgraph::Direction::Incoming);

...
impl Coordinator {
    ...
    async fn decompose_goal_into_tasks<F, Fut>(&self, goal: &str, llm_callback: &F) -> Result<TaskDag, String> 
    where
        ...
        Fut: std::future::Future<Output = Result<String, String>> + Send,
    {
        ...
        let json_text = raw_response.trim_start_matches("```json").trim_start_matches("```").trim_end_matches("```").trim();
        ...
        if let Some(tasks) = parsed.get("tasks").and_then(|t| t.as_array()) {
            for task_val in tasks {
                dag.add_task(Task {
                    ...
                    name: task_val["name"].as_str().unwrap_or("Unnamed Task").to_string(),
                    description: task_val["description"].as_str().unwrap_or("").to_string(),
                    assigned_agent: task_val["assigned_agent"].as_str().unwrap_or("general").to_string(),

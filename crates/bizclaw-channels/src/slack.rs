...
pub struct SlackChannel {
    ...
    connected: bool,
    /// Circuit breaker — prevents cascading failures when Slack API is down.
    ...
    async fn post_message(&self, channel: &str, text: &str, thread_ts: Option<&str>) -> Result<()> {
        if !self.circuit_breaker.can_execute() {
            return Err(BizClawError::Channel(format!(
                "Slack circuit breaker Open — message to {} rejected",
                channel
            )));
        }
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                return Err(BizClawError::Channel(format!("Slack API error: {e}")));
            }
        };

...
//!
//! Includes Circuit Breaker protection for Graph API calls.
...
pub struct WhatsAppChannel {
    ...
    connected: bool,
    /// Circuit breaker — prevents cascading failures when Graph API is down.
    ...
    async fn send_text_message(&self, to: &str, text: &str) -> Result<String> {
        if !self.circuit_breaker.can_execute() {
            return Err(BizClawError::Channel(format!(
                "WhatsApp circuit breaker Open — message to {} rejected",
                to
            )));
        }

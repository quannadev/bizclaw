...
pub fn decode_event_data(
    ...
    let decrypted_buffer = if encrypt_type != 1 {
        let cipher_key = cipher_key_b64.ok_or_else(|| anyhow::anyhow!("Missing cipher_key for encrypted payload"))?;
        ...
        cipher.decrypt(iv.into(), payload).map_err(|e| anyhow::anyhow!("AES-GCM Decrypt error: {:?}", e))?
    } else {
        decoded_buffer
    };
...
impl ParamsEncryptor {
    ...
    fn get_even_chars(s: &str) -> String {
        s.chars().enumerate().filter(|(i, _)| i % 2 == 0).map(|(_, c)| c).collect()
    }
    ...
    fn encode_aes_hex(key_str: &str, msg: &str) -> Option<String> {
        ...
        let enc = Aes256CbcEnc::new_from_slices(key, &iv).ok()?;
        let encoded_len = enc.encrypt_padded_mut::<Pkcs7>(&mut buf, pt_len).ok()?.len();

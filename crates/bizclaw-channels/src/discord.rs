...
//!
//! Includes Circuit Breaker protection for REST API calls.
...
pub struct DiscordChannel {
    ...
    connected: bool,
    /// Circuit breaker — prevents cascading failures when Discord API is down.
    ...
    pub async fn send_message(&self, channel_id: &str, content: &str) -> Result<()> {
        if !self.circuit_breaker.can_execute() {
            return Err(BizClawError::Channel(format!(
                "Discord circuit breaker Open — message to {} rejected",
                channel_id
            )));
        }
        ...
        let response = match self.client.post(&url).json(&body).send().await {
            ...
            Err(e) => {
                ...
                return Err(BizClawError::Channel(format!("Discord send failed: {e}")));
            }
        };

...
//!
//! Includes Circuit Breaker protection for outbound calls.
...
pub struct WebhookChannel {
    ...
    /// Circuit breaker — prevents cascading failures on outbound webhook.
        ...
        if let Some(url) = &self.config.outbound_url {
            if !self.circuit_breaker.can_execute() {
                return Err(BizClawError::Channel(
                    "Webhook circuit breaker Open — outbound rejected".into(),
                ));
            }
            ...
            match self.client.post(url).json(&body).send().await {
                ...
                Err(e) => {
                    self.circuit_breaker.record_failure();
                    return Err(BizClawError::Channel(format!("Webhook send failed: {e}")));
                }
            }
        }
        Ok(())
